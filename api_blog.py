...
from estrutura_banco_de_dados import Autor, Postagem, app, db           #importar do outro arquivo
import json 
import jwt
...
def token_obrigatorio(f):
    @wraps(f)                           #definir wraps de função 
    def decorated(*args, **kwargs):     #argumentos possicionais e arg. nomeados - independente de número de parametros
        token = None
        #Verificar se um token foi enviado 
        if 'x-access-token' in request.headers:      #armazendo informações extras 
            token = request.headers['x-access-token']
        if not token:
            return jsonify({'mensagem': 'Token não foi incluído!'}, 401)
        #Se temos um token, validar acesso consultando o BD
        try:                                    #decodificando e verificando se a inf. é uma inf. válida que está cadastrada
...
#Rota padrão - informação que recebe quando faz acesso ao URL base
@app.route('/')
@token_obrigatorio
...
def novo_autor(autor):
    novo_autor = request.get_json()                          #obter informações que estão vindo do REQUEST
    autor = Autor(nome=novo_autor['nome'], senha=novo_autor['senha'], email=novo_autor['email'])

    db.session.add(autor)                               #poder ser salvo no banco de dados
    db.session.commit()                                 #para salvar informações ao banco

    return jsonify({'mensagem': 'Usuário criado om sucesso'}, 200)
